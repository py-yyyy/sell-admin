...
function Order() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <div>
          <Button variant="filled" onClick={() => handleDetail(record)}>
            查看
          </Button>
          <Button
            color="primary"
  ...
  const handleSearch = () => {
    ...
    if (values.orderTime) {
      const s_time =
        values.orderTime[0].$y +
        "-" +
        (Number(values.orderTime[0].$M) + 1) +
        "-" +
        values.orderTime[0].$D;
      const e_time =
        values.orderTime[1].$y +
        "-" +
        (Number(values.orderTime[1].$M) + 1) +
        "-" +
        values.orderTime[1].$D;
  ...
  const getOrderDetail = async (id) => {
    ...
    setRecordDetail({
      ...
      orderTime: res.data.orderTime
        ? new Date(res.data.orderTime).toLocaleString()
        : "",
      deliveryTime: res.data.deliveryTime
        ? new Date(res.data.deliveryTime).toLocaleString()
        : "",
    });
    setItems([
      { label: "订单号", children: res.data.orderNo },
      { label: "联系电话", children: res.data.phone,span:"filled" },
      { label: "收货人", children: res.data.consignee },
      { label: "订单金额", children: <><span style={{ color: "red", fontWeight: "bold" }}>￥{res.data.orderAmount}</span></> },
      { label: "订单状态", children: <Tag
            color={
              res.data.orderState === "已完成"
                ? "green"
                : res.data.orderState === "已受理"
                  ? "blue"
                  : "orange"
            }
          >
      ...
      { label: "下单时间", children: res.data.orderTime? new Date(res.data.orderTime).toLocaleString() : "",span:"filled" },
      { label: "送达时间", children: res.data.deliveryTime? new Date(res.data.deliveryTime).toLocaleString() : "",span:"filled" },
      { label: "备注", children: res.data.remarks,span:"filled" },
      { label: "送货地址", children: res.data.deliverAddress,span:"filled" },
    ]);
  };
  ...
  const handleOkEdit = () => {
    ...
    data.deliveryTime = data.deliveryTime
      ? data.deliveryTime.format("YYYY/MM/DD HH:mm:ss")
      : "";
    if (
      !data.consignee ||
      !data.phone ||
          ...
          <Form.Item label="送达时间" name="deliveryTime">
            ...
            <DatePicker
              ...
              disabledDate={(currentDate) =>
                currentDate < dayjs(formEdit.getFieldValue("orderTime"))
              }
            />
          </Form.Item>
