...
import { message, Button, Form, Input, Select, DatePicker, Tag, Modal, Upload, Image, TimePicker, Checkbox } from 'antd';
...
function Shop() {
  ...
  const getShopInfo = async () => {
    ...
    setFileList(pics.map((url, index) => {
      return {
        uid: `pic-${index}`,
        name: `shop-pic-${index}`,
        status: 'done',
  ...
  const uploadButton = (
    <button style={{ border: 0, background: 'none' }} type="button">
      <div style={{ marginTop: 0, width: '200px', height: '200px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
  ...
  const handlePicsChange = ({ fileList: newFileList }) => {
    const filteredFileList = newFileList.filter(file => file.status !== 'uploading' && file.status !== 'removed');
    // 从fileList中提取图片URL
    const newPicsUrl = filteredFileList
      .map(file => file.response?.imgUrl || file.url || file.uid);
  ...
  const handleSubmit = async () => {
    ...
    data.date[0] = new Date(data.date[0]).toLocaleString();
    data.date[1] = new Date(data.date[1]).toLocaleString();
    ...
    <>
      <div className={styles.shop}>
        <PageHeader title="店铺信息" icon="icon-shangpin2" >
          <Button type="primary" color="primary" variant="filled" onClick={handleSubmit}>保存店铺信息</Button>
        </PageHeader>
        <div className={styles.shopContent}>
          <Form form={form} labelCol={{ span: 2 }}>
            <Form.Item name="name" label="店铺名称" >
              <Input placeholder="请输入店铺名称" style={{ width: '800px' }} />
            </Form.Item>
            <Form.Item name="bulletin" label="店铺公告">
              <Input.TextArea placeholder="请输入店铺公告" autoSize={{ minRows: 6, maxRows: 10 }} style={{ width: '800px' }} />
            </Form.Item>
            <Form.Item name="avatar" label="店铺头像">
              ...
              >
                {avatarUrl ? (
                  <img draggable={false} src={serverURL + avatarUrl} alt="image" style={{ width: '102px', height: '102px', borderRadius: 10 }} />
                ) : (
                  uploadButton
                )}
              </Upload>
            </Form.Item>
            <Form.Item name="pics" label="店铺图片">
              <div>
                {previewImage && (<Image
                  src={previewImage}
                  style={{ display: 'none' }}
                ...
                <Upload
                  ...
                  listType="picture-card"
                  className="image-uploader"
                  style={{ width: '102px', height: '102px', marginRight: 50, display: 'inline-block' }}
                  fileList={fileList}
            ...
            <Form.Item name="deliveryPrice" label="起送价格" >
              <Input placeholder="请输入起送价格" style={{ width: '800px' }} />
            </Form.Item>
            <Form.Item name="deliveryTime" label="送达时间" >
              <Input placeholder="请输入送达时间" style={{ width: '800px' }} />
            </Form.Item>
            <Form.Item name="description" label="配送描述" >
              <Input placeholder="请输入配送描述" style={{ width: '800px' }} />
            </Form.Item>
            <Form.Item name="score" label="店铺好评率" >
              <Input placeholder="请输入店铺好评率" style={{ width: '800px' }} />
            </Form.Item>
            <Form.Item name="sellCount" label="店铺销量" >
              <Input placeholder="请输入店铺销量" style={{ width: '800px' }} />
            </Form.Item>
            ...
            <Form.Item name="date" label="营业时间" >
              <TimePicker.RangePicker  style={{ width: '800px' }} />
            </Form.Item>
          </Form>
        </div>
      </div>
